...
\begin{remark}
	...
	\begin{itemize}
		\item (Objects) $\obj{\category}=\set{A,B,C,\dots}$
		\item (Morphisms) $\homo{A,B}=\set{f,g,\dots}$; $\homo{A,B}\neq\homo{B,A}$
		\item (Composition) \\ \adjustbox{scale=1.25, center}{
			\begin{tikzcd}
				A \arrow[rr, "f"]            &  & B \arrow[rr, "g"] &  & C \\
				A \arrow[rrrr, "h=g\circ f"] &  &                   &  & C
		\end{tikzcd}}
		\item (Identity) \[
		\id_A:A\to A
		\]
